...
to marry
 ...
 ; Identify the social network
  ...
  ; Evaluate the married fraction of social network
   ifelse count social-network = 0
      [set married-fraction 0]
      [set married-fraction (count social-network with [is-married?]) / (count social-network)]
   ...
   [
     hatch 1
       [
         ...
         ifelse ([my-spouses-social-angle] of myself) - ([ycor] of myself) >= 0
            [
              let big ([my-spouses-social-angle] of myself)
              let small ([ycor] of myself)
              ifelse big - small >= 20
                 [ifelse (random-float 1.0 < 0.5)
                   [set ycor (small - random-float small)]
                   [set ycor (big + random-float (360 - big))]
                 ]
                 [set ycor small + random-float(big - small)]  
            ]
            [
              let big ([my-spouses-social-angle] of myself)
              let small ([ycor] of myself)
